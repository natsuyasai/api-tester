...
const InteractiveFormDataEditor = (props: Omit<React.ComponentProps<typeof FormDataEditor>, 'onChange'>) => {
...
export const BulkEditMode: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    // バルク編集UIが表示されることを確認
    await expect(canvas.getByText('Enter one key-value pair per line in the format: key: value')).toBeInTheDocument()
